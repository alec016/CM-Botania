package es.degrassi.custommachinerybotania.client.integration.jei.mana;

import es.degrassi.custommachinerybotania.client.integration.jei.CustomIngredientTypes;
...
public class ManaJEIIngredientRenderer extends JEIIngredientRenderer<Mana, ManaGuiElement> {
    ...
    @Override
    public void render(@NotNull GuiGraphics matrix, @Nullable Mana ingredient) {
        int width = this.element.getWidth() - 4;
        int height = this.element.getHeight() - 2;

        ManaGuiElementWidget.renderMana(matrix.pose(), height, 0, 0, width, height);
    }
